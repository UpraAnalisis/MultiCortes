...
class ButtonMulticortes(object):
    """Implementation for Multicortes_addin.button (Button)"""
    def __init__(self):
        self.enabled = True
        self.checked = False
        sys.argv=['']
...
##                for carpeta in arreglo_rutas_excel:
##                    if tipo_analisis!="2":
##                        fusion(carpeta,carpeta.split("\\")[-1])
##                        grafica(carpeta+"\\"+carpeta.split("\\")[-1]+".xls")
##                    pass # si no hay estadisticas que no haga gráficas
